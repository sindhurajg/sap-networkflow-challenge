"""
To enable drill-down filtering by subscription,
resource group, NSG, protocol, and time window.
"""
...
def update_resource_groups(subscription):
    ...
    return [{'label': rg, 'value': rg}
            for rg in filtered['resource_group'].dropna().unique()]
...
def update_nsgs(subscription, rg):
    if not (subscription and rg):
        return []
    filtered = \
        df[(df['subscription'] == subscription) & (df['resource_group'] == rg)]
    return [{'label': nsg, 'value': nsg}
            for nsg in filtered['network_security_group'].dropna().unique()]
...
def filter_data(subscription, rg, nsg, protocol, start_date, end_date):
    ...
    if start_date and end_date:
        dff = dff[(dff['ft_time'] >= start_date) & (
            dff['ft_time'] <= end_date)]

    if dff.empty:
        return html.Div("⚠️ No records match the selected filters.")

    # Example chart: Number of flows over time
    fig = px.histogram(dff, x='ft_time', color='ft_protocol',
                       nbins=50, title="Filtered NSG Flows Over Time")
